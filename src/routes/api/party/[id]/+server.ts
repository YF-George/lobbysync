...
export async function PATCH({ params, request }) {
	...
	try {
		...
		let _localUserId: number | null = null;
		try {
			_localUserId = await mapAuthUserToDbUserId(authUser.id);
		} catch (_e) {
			// 無法對應 user，無權限編輯
		}

		const partyRows = await db
			.select({ owner: party.owner_id })
			.from(party)
			.where(eq(party.id, id));
		if (!partyRows || partyRows.length === 0)
			return json({ error: 'Party not found' }, { status: 404 });
		const _ownerId = partyRows[0].owner;
		...
		// 執行更新
		const result = await db.update(party).set(updateData).where(eq(party.id, id)).returning();
...
export async function DELETE({ params, request: _request }) {
	const _id = params.id;
	try {
		// Deleting parties is disabled in this deployment.
		return json({ error: 'Deleting parties is disabled' }, { status: 405 });
	} catch (err: any) {
