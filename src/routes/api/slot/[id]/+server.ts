...
export async function PATCH({ params, request }) {
	...
	if (Object.keys(update).length === 0)
		return json({ error: 'no updatable fields' }, { status: 400 });
	...
	if ('tags' in update) {
		...
		// 目前只允許 'leader' 標籤；其他類型在 UI/後端未使用，故移除以簡化資料結構。
		const validTags = ['leader'];
	...
	try {
		const result = await db.transaction(async (tx) => {
			...
			if (!parties || parties.length === 0)
				return { status: 404, body: { error: 'parent party not found' } };
			const p = parties[0];
			...
			if (!updated || updated.length === 0)
				return { status: 404, body: { error: 'update failed' } };

			return { status: 200, body: { slot: updated[0] } };
		});

		// 立即透過 Supabase Broadcast 推送，減少 WAL→Realtime 延遲
		if (result.status === 200 && result.body?.slot) {
			try {
				const updatedSlot = result.body.slot;
				const channel = supabaseAdmin.channel(`party:${updatedSlot.raid_id}`);
				await channel.subscribe();
				await channel.send({ type: 'broadcast', event: 'slot_updated', payload: updatedSlot });
				await channel.unsubscribe();
			} catch (broadcastErr) {
				console.error('Failed to broadcast slot update:', broadcastErr);
			}
		}

		// Database Changes 模式仍會觸發；兩種路徑並行以兼顧可靠性
